...
class LevelSampler():
    def __init__(
        ...
        self.seed_scores = np.array([0.]*len(seeds), dtype=np.float64)
        self.partial_seed_scores = np.zeros((num_actors, len(seeds)), dtype=np.float64)
        self.partial_seed_steps = np.zeros((num_actors, len(seeds)), dtype=np.int64)
        self.seed_staleness = np.array([0.]*len(seeds), dtype=np.float64)

        self.next_seed_index = 0 # Only used for sequential strategy

        # Logging
        self.sample_history = []  # list of (seed, is_replay) per sample call
    ...
    def _sample_replay_level(self):
        ...
        if np.isclose(np.sum(sample_weights), 0):
            sample_weights = np.ones_like(sample_weights, dtype=np.float64)/len(sample_weights)
        ...
        self.sample_history.append((int(seed), True))
        return int(seed)
    ...
    def _sample_unseen_level(self):
        ...
        self.sample_history.append((int(seed), False))
        return int(seed)
    ...
    def get_stats(self):
        """Return a dict of PLR stats for logging."""
        num_seen = int((self.unseen_seed_weights == 0).sum())
        num_unseen = len(self.seeds) - num_seen
        seen_mask = self.unseen_seed_weights == 0
        scores = self.seed_scores
        stats = {
            'plr/num_seen': num_seen,
            'plr/num_unseen': num_unseen,
            'plr/score_mean': float(scores[seen_mask].mean()) if num_seen > 0 else 0.0,
            'plr/score_max': float(scores[seen_mask].max()) if num_seen > 0 else 0.0,
            'plr/score_min': float(scores[seen_mask].min()) if num_seen > 0 else 0.0,
        }
        recent = self.sample_history[-100:] if self.sample_history else []
        if recent:
            replay_count = sum(1 for _, is_replay in recent if is_replay)
            stats['plr/replay_ratio_last100'] = replay_count / len(recent)
            seeds_sampled = [s for s, _ in recent]
            stats['plr/unique_seeds_last100'] = len(set(seeds_sampled))
        if num_seen > 0:
            top_idxs = np.argsort(scores)[-5:][::-1]
            for rank, idx in enumerate(top_idxs):
                stats[f'plr/top{rank+1}_seed'] = int(self.seeds[idx])
                stats[f'plr/top{rank+1}_score'] = float(scores[idx])
        return stats
