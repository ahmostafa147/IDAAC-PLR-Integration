...
class DAACRolloutStorage(RolloutStorage):
    ...
    def insert(self, obs, actions, action_log_probs,
               value_preds, rewards, masks, adv_preds):
        if len(rewards.shape) == 3: rewards = rewards.squeeze(2)
        self.obs[self.step + 1].copy_(obs)
        self.actions[self.step].copy_(actions)
        self.action_log_probs[self.step].copy_(action_log_probs)
...
class IDAACRolloutStorage(RolloutStorage):
    def __init__(self, num_steps, num_processes, obs_shape, action_space):
        ...
        # PLR compatibility: level_seeds [num_steps, num_processes, 1]
        # and action_log_dist [num_steps, num_processes, num_actions]
    ...
    def insert(self, obs, actions, action_log_probs, value_preds, \
               rewards, masks, adv_preds, levels, nsteps):
        if len(rewards.shape) == 3: rewards = rewards.squeeze(2)
        self.obs[self.step + 1].copy_(obs)
        self.actions[self.step].copy_(actions)
        self.action_log_probs[self.step].copy_(action_log_probs)
    ...
    def before_update(self):
        ...
        self.other_obs = other_obs.reshape(self.num_steps + 1, \
            self.num_processes, *self.obs.size()[2:])
        self.orders = torch.LongTensor(size=self.orders.shape).reshape(\
            (self.num_steps + 1)*self.num_processes).fill_(0).to(self.device)
