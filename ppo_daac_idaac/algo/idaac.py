...
class IDAAC():
    ...
    def __init__(self,
        ...
        self.clf_optimizer = optim.Adam(\
            self.order_classifier.parameters(), lr=lr, eps=eps)
    ...
    def update(self, rollouts):
        ...
        for e in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                obs_batch, actions_batch, value_preds_batch, return_batch, \
                    ...
                    other_obs_batch, clf_order = sample

                emb, adv, _, action_log_probs, dist_entropy = \
                    self.actor_critic.evaluate_actions(obs_batch, actions_batch)

                other_emb, _, _, _, _ = self.actor_critic.evaluate_actions(
                    other_obs_batch, actions_batch)
                
                emb_concat = torch.cat((emb, other_emb), dim=1)
                
                pred_order_clf = self.order_classifier(emb_concat.detach())
                clf_loss = self.bce_loss(pred_order_clf[:,0], \
                                         clf_order.squeeze().float())
                order_acc = (torch.argmax(pred_order_clf, dim=1) == \
                             clf_order.squeeze()).sum().item() / len(clf_order)
                self.clf_optimizer.zero_grad()
                clf_loss.backward()
                nn.utils.clip_grad_norm_(self.order_classifier.parameters(), \
                                         self.max_grad_norm)
                self.clf_optimizer.step()

                pred_order = self.order_classifier(emb_concat)       
                emb_order = torch.full(clf_order.shape, 0.5, device=clf_order.device)
        ...
        return order_acc_epoch, order_loss_epoch, clf_loss_epoch, \
            adv_loss_epoch, value_loss_epoch, action_loss_epoch, \
            dist_entropy_epoch
